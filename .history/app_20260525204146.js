...
async function pegarTodos() {
    ...
    container.innerHTML += `
            <div class="card" onclick="window.location.href='detail.html?id=123'" style="cursor: pointer">
                <img src=${entradas.imagem} alt="Nome do Produto" class="card-image" onerror="this.src='https://placehold.co/300x200?text=Sem+Imagem'">
                <div class="card-body">
                <span class="card-category text-muted">${entradas.categoria}</span>
                <h3 class="card-title">${entradas.nome}</h3>
                <p class="card-desc">${entradas.descricao}</p>
                <div class="card-footer">
                    <span class="price">R$ ${entradas.preco}</span>
