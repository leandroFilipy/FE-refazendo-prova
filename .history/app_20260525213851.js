...
async function pegarTodos() {
    ...
    const container3 = document.querySelector("#catalogo-wrapper")
    const container4 = document.querySelector("#catalogo-wrapper")
    ...
    container.innerHTML += `
        ...
        <!-- Cabeçalho da Seção com Título e Filtro de Preço -->
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 1rem; margin-bottom: 2rem;">
          <h2>pratos Principais <span style="font-size: 1rem; color: #888; font-weight: normal;"> - 2 itens</span></h2>
          <select class="seletor-ordem" style="padding: 0.5rem; border-radius: 4px; border: 1px solid #444; background-color: #222; color: #fff;">
            <option value="asc">Menor Preço</option>
            <option value="desc">Maior Preço</option>
          </select>
        ...
        `
        pratosPrincipais.forEach(produto => {
        ...
        <!-- Cabeçalho da Seção com Título e Filtro de Preço -->
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 1rem; margin-bottom: 2rem;">
          <h2>Bebidas <span style="font-size: 1rem; color: #888; font-weight: normal;"> - 2 itens</span></h2>
          <select class="seletor-ordem" style="padding: 0.5rem; border-radius: 4px; border: 1px solid #444; background-color: #222; color: #fff;">
            <option value="asc">Menor Preço</option>
            <option value="desc">Maior Preço</option>
          </select>
        ...
        <!-- Cabeçalho da Seção com Título e Filtro de Preço -->
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #333; padding-bottom: 1rem; margin-bottom: 2rem;">
          <h2>sobremesas <span style="font-size: 1rem; color: #888; font-weight: normal;"> - 2 itens</span></h2>
          <select class="seletor-ordem" style="padding: 0.5rem; border-radius: 4px; border: 1px solid #444; background-color: #222; color: #fff;">
            <option value="asc">Menor Preço</option>
            <option value="desc">Maior Preço</option>
          </select>
...
async function detalhes() {
    ...
    const container = document.querySelector("#info-prato-view")
    container.innerHTML=""
    const query = new URLSearchParams(window.location.search)
    const actualId = query.get("id")
    const resposta = await fetch(`https://api-restaurante-5iqb.onrender.com/api/produtos/${actualId}`)
    const data = await resposta.json()
    console.log(data)
    
    container.innerHTML += `
    <div class="detail-container">
        <img src=${data.imagem} alt="Nome do Produto" class="detail-image" onerror="this.src='https://placehold.co/600x400?text=Sem+Imagem'">
        <div class="detail-info">
          <span class="badge">${data.categoria}</span>
          <h1 style="margin-top: 1rem; font-size: 2.5rem;">${data.nome}</h1>
          <p class="detail-price">R$ ${data.preco.toFixed(2).replace(".", ",")}</p>
          <p style="font-size: 1.1rem; color: var(--text-muted); margin-bottom: 2rem;">
            ${data.descricao}
          </p>
          <p style="color: var(--text-muted); font-size: 0.9rem">ID: ${data.id}</p>
          ${data.destacado=== true ?  <p style="color: var(--primary-color); margin-top: 10px"><i class="fas fa-star"></i> Item em Destaque</p>`}
        </div>
      </div>
      `
